...
const Filter = ({ handleFilterChange }) => {
  ...
  // Desktop Filter Card
  const desktopFilter = (
    <div className="flex-col hidden w-64 p-4 transition-all duration-300 border shadow-md md:flex rounded-xl bg-white/90 backdrop-blur-sm dark:border-gray-800 dark:bg-gray-900/90">
      <FilterContent
        categories={categories}
        selectedCategories={selectedCategories}
        handleCategoryChange={handleCategoryChange}
        sortByPrice={sortByPrice}
        selectByPriceHandler={selectByPriceHandler}
      />
    </div>
  );
  ...
  const mobileFilter = (
    <Sheet>
      ...
      <SheetContent className="p-4 w-80">
        <SheetHeader>
...
// Separated component for filter content
const FilterContent = ({
  categories,
  selectedCategories,
  handleCategoryChange,
  sortByPrice,
  selectByPriceHandler,
}) => (
  <>
