...
const CourseDetail = () => {
  ...
  return (
    <div className="mt-16 space-y-10">
      ...
      <div className="bg-[#2D2F31] text-white">
        <div className="flex flex-col gap-2 px-4 py-8 mx-auto max-w-7xl md:px-8">
          {/* 👇 Step 1: Schema ke anusaar names ko theek karein */}
          ...
          <p className="text-base md:text-lg">
            Created By{" "}
            <span className="text-[#C0C4FC] underline italic">
              {/* `createdBy` ko `creator` se badlein */}
              {course.creator.name}
            </span>
          </p>
          <div className="flex items-center gap-2 mt-1 text-sm">
          ...
          {/* `students` ko `enrolledStudents` se badlein */}
          <p className="mt-1 text-sm">Students enrolled: {course.enrolledStudents?.length || 0}+</p>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-10 px-4 mx-auto my-5 max-w-7xl md:px-8 lg:flex-row">
            ...
            <CardContent className="space-y-3">
              {(course.lectures || []).map((lecture, idx) => (
                ...
                >
                  ...
                  {/* Lecture title ka naam theek karein */}
        ...
        <div className="w-full space-y-5 lg:w-1/3">
          <Card>
            <CardContent className="flex flex-col p-4">
              <div className="flex items-center justify-center w-full mb-4 text-white bg-black aspect-video">
                {/* `thumbnail.url` ki jagah seedha `courseThumbnail` use karein */}
                <img src={course.courseThumbnail} alt={course.courseTitle} className="object-cover w-full h-full" />
              </div>
              <Separator className="my-2" />
              <h3 className="text-lg font-semibold md:text-xl">
                {/* `price` ko `coursePrice` se badlein */}
