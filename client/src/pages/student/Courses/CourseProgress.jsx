...
const CourseProgress = () => {
  ...
  if (isError) return <p className="p-4">Failed to load course details</p>;

  if (!data?.data) {
    return <div className="p-4">No course progress data available.</div>;
  }
  ...
  const handleLectureProgress = async (lectureId) => {
    try {
      console.log(
        `Updating progress: courseId=${courseId}, lectureId=${lectureId}`
      ); // Debug
